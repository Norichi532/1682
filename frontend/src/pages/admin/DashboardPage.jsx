  ...
  if (active && payload && payload.length) {
    return (
      <div className="bg-white border border-gray-100 shadow-lg rounded-xl px-4 py-3 text-sm">
        ...
        <p className="text-gray-400">{payload[1]?.value || 0} đơn</p>
      </div>
    )
  }
  return null
  ...
  if (active && payload && payload.length) {
    return (
      <div className="bg-white border border-gray-100 shadow-lg rounded-xl px-4 py-3 text-sm">
        ...
        <p className="text-navy font-bold">{payload[0].value} đơn</p>
      </div>
    )
  }
  return null
...
export default function DashboardPage() {
  ...
  // ── Doanh thu + số đơn theo tháng (năm hiện tại) ────────────────────────────
  ...
  // ── Tỷ lệ trạng thái đơn (Pie chart) ────────────────────────────────────────
      ...
      <div className="space-y-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          <StatCard
            ...
            title="Hoàn thành"
            value={completed}
            sub={`Tỷ lệ ${total ? Math.round(completed/total*100) : 0}%`}
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
          <h2 className="font-display text-lg font-bold text-navy mb-6">
            Doanh thu & Số đơn theo tháng ({currentYear})
          </h2>
          <ResponsiveContainer width="100%" height={280}>
            <BarChart data={monthlyData} barGap={4}>
              ...
              <Bar yAxisId="left" dataKey="revenue" name="Doanh thu" fill="#C9A84C" radius={[6,6,0,0]} />
              <Bar yAxisId="right" dataKey="orders" name="Số đơn" fill="#1B2A4A" radius={[6,6,0,0]} opacity={0.7} />
            </BarChart>
          </ResponsiveContainer>
          <div className="flex items-center gap-6 mt-4 justify-center">
            <div className="flex items-center gap-2 text-sm text-gray-500">
              <div className="w-3 h-3 rounded-sm bg-ochre" /> Doanh thu
            </div>
            <div className="flex items-center gap-2 text-sm text-gray-500">
              <div className="w-3 h-3 rounded-sm bg-navy opacity-70" /> Số đơn
            </div>
          </div>
        </div>

        {/* ── Pie chart + bảng gần nhất ── */}
        <div className="grid grid-cols-1 lg:grid-cols-5 gap-6">

          {/* Pie chart tỷ lệ trạng thái */}
          <div className="lg:col-span-2 bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
            <h2 className="font-display text-lg font-bold text-navy mb-4">Tỷ lệ trạng thái đơn</h2>
            {pieData.length === 0 ? (
          ...
          <div className="lg:col-span-3 bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
            <h2 className="font-display text-lg font-bold text-navy mb-4">Đơn gần nhất</h2>
            <div className="space-y-3">
              {recent.length === 0 ? (
                <p className="text-gray-400 text-sm text-center py-8">Chưa có đơn nào</p>
              ) : recent.map(b => (
